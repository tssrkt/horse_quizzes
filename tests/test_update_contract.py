...
class UpdateContractTests(unittest.TestCase):
    ...
    def test_catalog_card_has_difficulty_before_tags_and_description_is_three_pixels_larger(self):
        ...
        self.assertIn("englishSite ? 'Difficulty' : 'Сложность'", javascript)
        self.assertIn("DIFFICULTY_LABELS[quiz.difficulty]", javascript)
    ...
    def test_result_page_renders_optional_next_quiz_link(self):
        ...
        self.assertIn("englishSite ? 'Next quiz' : 'Следующая викторина'", javascript)
        self.assertIn("core.quizPath(nextQuiz.public_slug || nextQuiz.slug, location.href)", javascript)
