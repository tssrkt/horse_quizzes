...
function init(core) {
  ...
  function renderResult() {
    ...
    const recommendation = englishSite ? (percent === 100 ? 'You answered every question correctly and clearly know this subject extremely well. You’re not so easy to fool! And there’s sure to be plenty more interesting information waiting for you in the collection of articles about horses.' : percent >= 75 ? 'Great result! You know the subject well and are already very close to perfection. The collection of articles about horses contains plenty of other interesting facts that can help fill in the remaining gaps and perhaps help you answer every question correctly next time.' : percent >= 50 ? 'Not a bad result! You already know quite a lot about horses, although some of the questions still proved challenging. If you’d like to explore the topic in greater depth, take a look at the collection of articles and then try the quiz again. Chances are, your result will be even better next time.' : 'Well, some of the questions turned out to be quite challenging — and that’s a great reason to learn more! If you’d like to explore the topic in greater depth, take a look at the collection of articles about horses and then try the quiz again. Chances are, your result will pleasantly surprise you next time.') : core.resultRecommendation(percent);
